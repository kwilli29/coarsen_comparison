...
int* kokkos_mis(struct Graph* G){ // 3. Build MIS of supernodes roughly based off Kokkos paper
    ...
    while (wrk1sz > 0){ // while there exist undecided vertices 
        ...
        for(int i = 0; i < wrk1sz; i++){
            refreshRowStatus(rowStatus, worklist1[i], nvbits, numVerts); // 3.a. Give random priorities to the undecided vertices
        }

        for(int i = 0; i < wrk2sz; i++){
            refreshColStatus(colStatus, rowStatus, worklist2[i], numVerts, G); // 3.b. Mark local minima
        }
...
void thirdPass(struct Graph* G, int unasgn, int* labels, int* supernode_ptr){ // 6. Third Pass
...
struct Graph* build_edges(struct Graph* G, int* labels, int spn){ // 7. Build Edges
    ...
    for(int k=0; k < spn; k++){  memset(edge_matrix[k],0,spn*sizeof(float)); }
