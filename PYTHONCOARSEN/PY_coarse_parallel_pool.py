...
def kokkos_mis(G): # 3. Build MIS of supernodes based off Kokkos paper #
    ...
    while worklist1:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=500) as executor:
            for x,i in enumerate(worklist1): #seq. or parallel??
                executor.submit(refreshRowStatus, rowStatus, i, iter, nvbits, numVerts, lock)

        # sync
        with concurrent.futures.ThreadPoolExecutor(max_workers=5000) as executor:
            for x,i in enumerate(worklist2):
                executor.submit(refreshColStatus, colStatus, rowStatus, i,  numVerts, G)
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=500) as executor:
            for x,i in enumerate(worklist1):
                executor.submit(decideInOut, i,rowStatus,colStatus,G,lock)
...
def decideInOut(i, rowStatus, colStatus, G, lock): # 3.c. compare neighbors and see which vertices can be IN or OUT
...
def secondPass(G, unasgn, lock):#, vertex_set): # 5. Unassigned Neighbors of RootNeighbors = Supernode
    #print('5')
...
def thirdPass(G, unasgn,lock):# vertex_set): # 6. Still Unassigned? New Supernode & Reassign Neighbors
    #print('6')
...
def kokkos_coarsen(G): # 2. Kokkos Coarsen #
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=500) as executor:
        for supernode, m in enumerate(M1): #seq. or parallel??
            executor.submit(firstPass, G,supernode,m)
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=500) as executor:
        for x, unasgn in enumerate(vertex_set): #seq. or parallel??
            executor.submit(secondPass, G,unasgn,lock2)
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=500) as executor:
        for x, unasgn in enumerate(vertex_set): #seq. or parallel??
            executor.submit(thirdPass, G, unasgn,lock3)
